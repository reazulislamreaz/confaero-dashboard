...
//   // ── FIXED: correctly maps _id → id and normalises date ──────────────────
...
//       // slice ISO string to yyyy-MM-dd for <input type="date">
//       date: session.date
...
//     } else {      toast.error('Failed to delete session. Please try again.');
//     }
...
//       // Send proper ISO string to the server
...
//           sessionId: editingSession.id,   // correctly set from session._id in handleEdit
//           eventId,
//           session: formData,
//         });
//       } else {
...
//       if (result?.data?.success === true) {
//         toast.success(
//           editingSession.id ? 'Session updated successfully!' : 'Session added successfully!'
//         );
...
//           (editingSession.id ? 'Failed to update session.' : 'Failed to add session.')
//         );
//       }
//     } catch (error) {
...
//           <input type="datetime-local" value={eventData.startDate}
//             onChange={(e) => setEventData((p) => ({ ...p, startDate: e.target.value }))}
//             className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500" />
//         </div>
//         <div>
//           <label className="block text-sm font-medium text-gray-700 mb-2">End Date & Time</label>
//           <input type="datetime-local" value={eventData.endDate}
...
//           <input type="text" value={eventData.mapLink}
...
//         <input type="text" value={eventData.website}
...
//         <input type="text" value={floorMapData.title}
...
//                 <div className="mb-4">
//                   <h3 className="text-sm font-semibold text-gray-800 mb-2">Floor Map</h3>
//                   <img src={selectedSession.floorMapUrl} alt="Floor map" className="w-full rounded-lg object-cover max-h-48" />
//                 </div>
//               )}
//             </div>
...
const isError = isAdmin ? adminError : userError;
  ...
  const handleConfirmCsvUpload = async () => {
    ...
    try {
      ...
      const result = await addSessionByCsvfile({ eventId, file: formData });
      ...
      if (result?.data?.success === true) {
        ...
        toast.error(
          result?.error?.data?.message || 'Failed to import CSV. Please try again.'
        );
      }
    } catch (error) {
      console.error('CSV upload error:', error);
      toast.error('An unexpected error occurred while uploading the CSV.');
    } finally {
          ...
          {!csvFile ? (
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1.5 px-3 py-2 bg-teal-50 border border-teal-200 rounded-lg text-sm text-teal-700 max-w-[180px]">
                ...
                <button
                  onClick={() => { setCsvFile(null); setCsvFileName(''); }}
