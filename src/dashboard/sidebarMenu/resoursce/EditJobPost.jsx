...
import { Upload, X, Bold, Italic, Underline, Link2, List, ListOrdered, ArrowLeft } from 'lucide-react';
...
export default function EditJobPost() {
  ...
  // RTK Query hooks
  const { data: jobData, isLoading: jobLoading, refetch } = useGetJobByIdQuery(id, { skip: !id });
          ...
          <div>
              ...
              {bannerPreview ? (
                <div className="relative">
                  ...
                  <button
                    onClick={() => {
                      setBannerImage(null);
                      setBannerPreview(bannerUrl); // Revert to saved URL
                    }}
                    className="absolute top-2 right-2 bg-red-500 text-white rounded-full p-1 hover:bg-red-600"
                  >
                    <X className="w-4 h-4" />
            ...
            {bannerImage && !bannerUrl && (
              <button
                ...
                className="mt-3 w-full px-4 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700 disabled:opacity-50 transition-colors"
              >
                {isUploading ? 'Uploading...' : 'Upload Image to Server'}
              </button>
            )}
            
            {/* Show status if uploaded */}
